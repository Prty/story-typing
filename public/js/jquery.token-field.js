...
(function($) {
  $.fn.tokenField = function(options) {
        ...
        .blur(function(e) {
          if (!$(this).data('blur')) {
            ...
            if (settings.max == 0 || $(this).closest('.token-input').siblings('.token').length < settings.max) {
              if (isToken($(this).val())) {
                var wordState;
                if ($.inArray($(this).val(), observed_WORDS) > -1) {
                  property = $(this).val();
                  wordState = 'keyword';
                } else {
                  property = 'none';
                ...
                observeToken($(tokenHtml($(this).attr('name'), $(this).val(), property, wordState)).insertBefore($(this).closest('.token-input')));
                if (!settings.nested) {
    ...
    function tokenHtml(name,text, property, wordState) {
      return "<a href='#' class='token'><span><span><span><span class='image-span-wrapper " + wordState + "'><div class='image-span " + property + "'></div>"
              + text
              + (settings.nested ? "<input type='hidden' value='"+text+"' name='"+name+"'/>" : "")
              + "<span href='#' class='token-x'></span></span></span></span></span></a>";
    }
